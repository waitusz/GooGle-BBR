...
Install()
{
    ...
    if uname -r | grep -Eqi "4.9."; then
		...
		if [ $code = "y" -o $code = "Y" ]; then
		    if [ $OS_Bit = "64" ]; then
		        rpm -ivh https://github.com/waitusz/GooGle-BBR/raw/master/kernel/kernel-ml-4.9.0-1.el6.elrepo.x86_64.rpm --force
			fi
			if [ $OS_Bit = "32" ]; then
		        rpm -ivh https://github.com/waitusz/GooGle-BBR/raw/master/kernel/kernel-ml-4.9.0-1.el6.elrepo.i686.rpm --force
			fi
